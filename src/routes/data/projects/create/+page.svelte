   ...
   <form>
      ...
      <div class="field">
         ...
         <div class="article-input">
            <input
               type="text"
               id="article-search"
               bind:value={articleSearchText}
            />
            <button on:click={() => searchForArticle(articleSearchText)}
               >Search</button
            >
         </div>
         {#if articleSearchResults.length > 0}
            <ul class="search-results">
               {#each articleSearchResults as article}
                  <li
                     class="result-item"
                     on:click={(event) => selectMainArticle(event, article)}
                  >
                     {article.title}
                  </li>
               {/each}
      ...
      <div class="button-group">
         ...
         <button
            ...
            type="submit"
            on:click|preventDefault={handleSubmit}>Add Article</button
         >
      </div>
   </form>
</div>
