...
<form on:submit={createTag} class="flex">
  ...
  <button type="submit" class="inline-block whitespace-nowrap px-3 py-1 bg-white">Create Tag</button>
</form>

<div class="tag-list">
  {#each articleTags as tag}
      <div class="mt-2">{tag.title} - <span class="clickable" on:click={() => deleteTag(tag._id)}>Delete</span></div>
  {/each}
</div>
